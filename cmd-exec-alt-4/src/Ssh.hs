...
instance Show SshCommand where
  show (Ssh creds cmd) = wrapSshCommand creds cmd

wrapSshCommand :: SshCredentials -> Command -> String
wrapSshCommand creds cmd = "ssh " <> showSshOptions creds <> " '" ++ escapeQuotes cmd <> "'"
